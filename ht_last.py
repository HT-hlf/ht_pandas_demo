...
def process(list_process,Performance,Culture):
    if x_min<=Performance<x_min+x_d and y_min<=Culture<y_min+y_d:
        list_process[0]=list_process[0]+1
    elif x_min+x_d<=Performance<x_min+2*x_d and y_min<=Culture<y_min+y_d:
        list_process[1]=list_process[1]+1
    elif x_min+2*x_d<=Performance and y_min<=Culture<y_min+y_d:
        list_process[3]=list_process[3]+1
    elif x_min<=Performance<x_min+x_d and y_min+y_d<=Culture<y_min+2*y_d:
        list_process[2]=list_process[2]+1
    elif x_min+x_d<=Performance<x_min+2*x_d and y_min+y_d<=Culture<y_min+2*y_d:
        list_process[5]=list_process[5]+1
    elif x_min+2*x_d<=Performance and y_min+y_d<=Culture<y_min+2*y_d:
        list_process[6]=list_process[6]+1
    elif x_min<=Performance<x_min+x_d and y_min+2*y_d<=Culture:
        list_process[4]=list_process[4]+1
    elif x_min+x_d<=Performance<x_min+2*x_d and y_min+2*y_d<=Culture:
        list_process[7]=list_process[7]+1
    elif x_min+2*x_d<=Performance and y_min+2*y_d<=Culture:
        list_process[8]=list_process[8]+1
    else:
        print('process error!!!')
    return list_process
...
def people(name,first_index='Unknow'):
    ...
    sheet_index2=[sheet_index1[0]+table_num]
    ...
    list_process=[0,0,0,0,0,0,0,0,0]
    list_process_ag=[]
    list_process_ag_p=[]
    list_process_ag_a=[]
    for i in range(month_num):
        ...
        list_process=process(list_process,Performance,Culture)

        list_process_ag_p.append(Performance)
        ag_p=sum(list_process_ag_p)/len(list_process_ag_p)

        list_process_ag_a.append(Culture)
        ag_a=sum(list_process_ag_a)/len(list_process_ag_a)

        list_process_ag.append(Performance_Culture)
        ag=sum(list_process_ag)/len(list_process_ag)
        # ag=mean(list_process_ag)
...
list_month=['指标分类1','指标名称']+month_list[0:month_num]
print(list_month)
#月份设置  
data = pd.read_excel(io, sheet_name = 'L3&VS-Assy',header=2,usecols=list_month)
data_1=data.copy()
