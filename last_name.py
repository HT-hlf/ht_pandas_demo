...
io_yongyv= r'extract_主管领导力特质评分表.xlsx'
io_PID = r'.\extract_主管级PID审核分值统计表.xlsx'
    ...
    for i in range(10):
        ...
        if len(df_PID[df_PID['名字'].isin([name]) & df_PID['月份'].isin([i+1])]['得分'].values)!=0:
            ...
            if pd.isnull(value_PID):
                ...
                if value_PID>=85:
                    list_1.append(100)
                    write_txt_s('[{}] read data:{}, 实际得分：{}\n'.format(month_list[i],value_PID,100))
                else:
                    list_1_n=value_PID/85*100
                    list_1.append(list_1_n)
                    write_txt_s('[{}] read data:{}, 实际得分：{}\n'.format(month_list[i],value_PID,list_1_n))
            else:
...
def yongyv(name):
    ...
    for i in range(10):
        ...
        if len(df_yongyv[df_yongyv['名字'].isin([name]) & df_yongyv['月份'].isin([i+1])]['得分'].values)!=0:
            ...
            print('{} num error!'.format(month_list[i]))
            write_txt_s('{} Num Error!'.format(month_list[i]))
        write_txt_s('[{}] read data:{}, 实际得分：{}\n'.format(month_list[i],value_yongyv,list_1[-1]))
    return list_1
...
# data_PID = pd.read_excel(io_PID,sheet_name='主管级PID 审核分值统计-10月',header=2,usecols=['主管','平均分'])
# data_PID=data_PID.dropna()
# #有些人没数据
...
df_PID = pd.read_excel(io_PID)

df_yongyv=pd.read_excel(io_yongyv)
...
people('李保平','LHEX- SUB1')
people('沈波','LHEX- SUB2')
people('邢菲菲','LHEX- UF')
people('魏小恒','LHEX- BF')
people('马浩','LHEX- Front',)
people('姚峰','SLHEX- 分装')
people('周百顺','SLHEX- 1')
people('张震','SLHEX- 2')
